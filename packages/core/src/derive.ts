...
export function deriveExportJob(
  ...
  return {
    record: job,
    storedContainers: [...containers],
    journey: ctx ? exportJourney({
      mandatoryComplete: ctx.mandatoryComplete,
      missingFields: missing,
      cmsRequired: ctx.cmsRequired,
      cmsCompleted: ctx.cmsCompleted,
      emptyGatePassed: ctx.emptyGatePassed,
      emptyScheduled: ctx.emptyScheduled,
      emptyCollected: ctx.emptyCollected,
      emptyDelivered: ctx.emptyDelivered,
      containerNumberCaptured: ctx.containerNumberCaptured,
      detailsSent: ctx.detailsSent,
      containerReady: ctx.containerReady,
      vgmReceived: ctx.vgmReceived,
      transhipmentStatus: ctx.transhipmentStatus,
      carparkRequested: ctx.carparkRequested,
      atCarpark: ctx.atCarpark,
      ladenGatePassed: ctx.ladenGatePassed,
      hasLadenMovement: ctx.hasLadenMovement,
      deliveredToPort: ctx.deliveredToPort,
      jobClosed: Boolean(job.closedAt),
    }) : [],
