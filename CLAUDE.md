...
greenlit-site     Next.js 15 app, deployed to Vercel. 26 tests.
...
no deep imports into `src/`. All three are npm workspace members, so
`@greenlit/*` resolve natively; Next compiles them via `transpilePackages`
because ADR-0006 keeps them as buildless TypeScript source.
...
- **Deployment is Vercel** (ADR-0008), built from the repo root because the
  app imports packages that live outside its directory.
